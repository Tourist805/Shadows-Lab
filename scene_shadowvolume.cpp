...
Scene_ShadowVolume::Scene_ShadowVolume() : rotSpeed(0.1f), tPrev(0),
plane(10.0f, 10.0f, 2, 2, 5.0f, 5.0f)
{
	spot = ObjMesh::loadWithAdjacency("../Shadows-Lab/media/spot/spot_triangulated.obj");
}

void Scene_ShadowVolume::initScene()
{
	compile();
	...
	// Set up the VAO for the full-screen quad
...
void Scene_ShadowVolume::updateLight()
{
	// World position
	lightPos = vec4(5.0f * vec3(cosf(angle) * 7.5f, 1.5f, sinf(angle) * 7.5f), 1.0f);
}

void Scene_ShadowVolume::setupFBO()
{
	// The depth buffer
	GLuint depthBuf;
	...
	glGenFramebuffers(1, &colourDepthFBO);
	glBindFramebuffer(GL_FRAMEBUFFER, colourDepthFBO);
...
void Scene_ShadowVolume::update(float t)
{
...
void Scene_ShadowVolume::render()
{
	pass1();
	glFlush();
	pass2();
...
void Scene_ShadowVolume::pass2() {
	...
	glBindFramebuffer(GL_READ_FRAMEBUFFER, colourDepthFBO);
...
void Scene_ShadowVolume::drawScene(GLSLProgram& prog, bool onlyShadowCasters)
{
	vec3 color;
...
void Scene_ShadowVolume::setMatrices(GLSLProgram &prog)
{
	mat4 mv = view * model;
...
void Scene_ShadowVolume::resize(int w, int h)
{
	glViewport(0, 0, w, h);
	width = w;
	height = h;
...
void Scene_ShadowVolume::compile()
{
	try
	{
		// The shader for the volumes
		volumeProg.compileShader("shader/shadow_volume.vert");
		volumeProg.compileShader("shader/shadow_volume.geom");
		volumeProg.compileShader("shader/shadow_volume.frag");
		volumeProg.link();
		...
		compProg.compileShader("shader/shadow_volume_compute.vert");
		compProg.compileShader("shader/shadow_volume_compute.frag");
		compProg.link();
	}
