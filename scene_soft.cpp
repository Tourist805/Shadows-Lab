...
Scene_Soft::Scene_Soft() : plane(20.0f, 20.0f, 2, 2)
{
	shadowMapWidth = 512;
	shadowMapHeight = 512;
...
void Scene_Soft::buildJitterTex()
{
...
void Scene_Soft::update(float t)
{
	angle += 0.003f;
...
void Scene_Soft::render()
{
	// Pass 1 (shadow map generator)
	...
	view = glm::lookAt(cameraPos, vec3(0.0f, -0.175f, 0.0f), vec3(0.0, 1.0f, 0.0f));
...
void Scene_Soft::drawBuildingScene()
{
...
void Scene_Soft::resize(int w, int h)
{
	glViewport(0, 0, w, h);
	width = w;
	height = h;
...
void Scene_Soft::compile()
{
	try {
		prog.compileShader("shader/soft_shadows.vert");
		prog.compileShader("shader/soft_shadows.frag");
		prog.link();
		prog.use();
	}
