...
Scene_Transparency::Scene_Transparency() : angle(0.0f), tPrev(0.0f),
rotSpeed(glm::pi<float>() / 8.0f), sphere(1.0f, 40, 40)
{ }

void Scene_Transparency::initScene()
{
    compile();
    animate(false);
...
void Scene_Transparency::clearBuffers() {
    GLuint zero = 0;
...
void Scene_Transparency::setMatrices()
{
    mat4 mv = view * model;
...
void Scene_Transparency::resize(int w, int h)
{
    glViewport(0, 0, w, h);
    width = w;
    height = h;
...
void Scene_Transparency::compile()
{
    try {
        prog.compileShader("shader/independent_transp.vert");
        prog.compileShader("shader/independent_transp.frag");
        prog.link();
        prog.use();
    }
