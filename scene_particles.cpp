...
Scene_Particles::Scene_Particles() :
    ...
    bh1(5, 0, 0, 1), bh2(-5, 0, 0, 1), bh3(-2.5, 0, 0, 1)
{
    totalParticles = nParticles.x * nParticles.y * nParticles.z;
}

void Scene_Particles::initScene()
{
    compile();
    initBuffers();
...
void Scene_Particles::initBuffers()
{
    ...
    GLfloat data[] = { bh1.x, bh1.y, bh1.z, bh1.w, bh2.x, bh2.y, bh2.z, bh2.w, bh3.x, bh3.y, bh3.z, bh3.w };
...
void Scene_Particles::update(float t)
{
    if (time == 0.0f) {
        deltaT = 0.0f;
    }
...
void Scene_Particles::render()
{
    ...
    // Draw the attractors
    glPointSize(5.0f);
    GLfloat data[] = { att1.x, att1.y, att1.z, 1.0f, att2.x, att2.y, att2.z, att3.x, att3.y, att3.z, 1.0f };
    glBindBuffer(GL_ARRAY_BUFFER, bhBuf);
...
void Scene_Particles::setMatrices() {
    renderProg.use();
...
void Scene_Particles::resize(int w, int h)
{
    glViewport(0, 0, w, h);
    width = w;
    height = h;
...
void Scene_Particles::compile()
{
    try {
        renderProg.compileShader("shader/particles.vert");
        renderProg.compileShader("shader/particles.frag");
        renderProg.link();

        computeProg.compileShader("shader/particles.comp");
        computeProg.link();
    }
